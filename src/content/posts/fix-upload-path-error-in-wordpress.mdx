...
Has it ever happened to you that after a migration or after [migrating the site](/increase-upload-limit-for-all-in-one-wp-migration-plugin/) the [WordPress](/wordpress/) media images no longer work correctly?

Usually this is happen, because the media path has changed. And most of us we face it after doing [WordPress website migration](/increase-upload-limit-for-all-in-one-wp-migration-plugin/), either Manual WordPress website migration or Migration using any WordPress Plugin.
...
Unfortunately in current versions of [WordPress](/learn-wordpress-development-the-right-way/) it is not included to change the media path through the configuration screen, so if you get complicated with the code you can evaluate using a plugin.

The plugin that we will use will be [WP Original Media Path](https://wordpress.org/plugins/wp-original-media-path/).

[![](/uploads/wp-original-media-path-1024x463.png)](/fix-upload-path-error-in-wordpress/wp-original-media-path/)

After downloading and installing the plugin we will see a new option in the Settings menu:

[![](/uploads/wp-original-media-path-setting-1024x468.png)](/fix-upload-path-error-in-wordpress/wp-original-media-path-setting/)
