...
If you're hitting the upload size limit on the destination rather than the size of the export itself, that's a separate problem. I wrote a tiny plugin to fix it, and a [post on how to lift the upload limit on All-in-One WP Migration](/increase-upload-limit-for-all-in-one-wp-migration-plugin/) that walks through it.
