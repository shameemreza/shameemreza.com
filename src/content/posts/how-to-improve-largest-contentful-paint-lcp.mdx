...
Client-side rendering is common, but it hurts LCP because the browser has to download and run JavaScript before any of the hero content shows up. If you're using a framework like Next.js or Remix, server-side rendering or static generation usually gives you a better LCP for free.
...
For automatic compression and WebP conversion on upload, ShortPixel and the core WordPress lazy loading (enabled by default since 5.5) cover most of what you need.
...
## A short note on priorities

If you're only going to fix two things, fix your host and your LCP image. Those two changes tend to move the number more than anything else. Everything in this list is useful, but they compound on top of a fast server and an optimized hero image, not in place of them.

Minification, unused-CSS pruning, and service workers are real wins, but they're second-order. A slow host with a 3 MB PNG as the LCP element will still score poorly no matter how aggressively you minify your JavaScript.
